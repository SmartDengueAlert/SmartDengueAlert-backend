...
const preprocessWeatherNDengueData = async (weatherData,dengueData) => {
    ...
    const denguecases = dengueData.map(day => day.cases);
    const rollingMean = denguecases.reduce((a, b) => a + b, 0) / denguecases.length;
    const rollingStd = Math.sqrt(denguecases.map(x => Math.pow(x - rollingMean, 2)).reduce((a, b) => a + b) / denguecases.length);

    // printing denguedeaths,rolling mean and rolling std
     console.log('**dengue_cases_table: ',dengueData);
     console.log('--Dengue cases in latest 14 days: ',denguecases);
     console.log('**Rolling mean of dengue cases: ', rollingMean);
     console.log('--Rolling std of dengue cases: ', rollingStd);
